...
export default function TableOfContentsBlock({
  ...
  const [tableOfContentsContent,setTableOfContentsContent,]=useState<{typeOfBlock:typeOfBlockType, blockContent: string, number: ReactElement}[]|null|undefined>()

  useEffect(()=>{
    let sectionCounter=0
    let subsectionCounter=0
    let subsubsectionCounter=0
    const sections =blocksContent.filter(block=> block.typeOfBlock==='section' || block.typeOfBlock==='subsection' || block.typeOfBlock==='subsubsection')
    const tableOfContentsData = sections.map((item)=>{
      if(item.typeOfBlock==='section'){
        sectionCounter+=1
        subsectionCounter=0
        subsubsectionCounter=0
        const section= {typeOfBlock: item.typeOfBlock, blockContent: item.blockContent as string, number: <Text fw='bold' mr='xs'>{sectionCounter.toString()+'.'}</Text>}
      ...
      if(item.typeOfBlock==='subsection'){
        subsectionCounter+=1
        subsubsectionCounter=0
        const subsection=  {typeOfBlock: item.typeOfBlock, blockContent: item.blockContent, number: <Text  fw='bold' mr='xs' ml='md'>{sectionCounter.toString()+'.'+subsectionCounter.toString()+'.'}</Text>}
        
        return subsection
      }
      if(item.typeOfBlock==='subsubsection'){
        subsubsectionCounter+=1
        const subsubsection=  {typeOfBlock: item.typeOfBlock, blockContent: item.blockContent, number: <Text  fw='bold' mr='xs'  ml='xl'>{sectionCounter.toString()+'.'+subsectionCounter.toString()+'.'+subsubsectionCounter.toString()+'.'}</Text>}
